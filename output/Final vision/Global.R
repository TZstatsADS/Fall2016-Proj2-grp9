...
Nearest3<-function(xlat,xlon,slat,slon,sname,nuba,nuda)
{ 
  distance<-vector(length=length(slat))
  for(i in 1:length(slat))
  {distance[i]<-sqrt((xlat-slat[i])^2+(xlon-slon[i])^2)}
  rdis<-rank(distance)
  p1<-c(slat[rdis==1],slon[rdis==1],nuba[rdis==1],nuda[rdis==1],sname[rdis==1])
  p2<-c(slat[rdis==2],slon[rdis==2],nuba[rdis==2],nuda[rdis==2],sname[rdis==2])
  p3<-c(slat[rdis==3],slon[rdis==3],nuba[rdis==3],nuda[rdis==3],sname[rdis==3])
